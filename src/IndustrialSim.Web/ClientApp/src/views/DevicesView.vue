...
const workspace = useWorkspaceStore(); const devices = ref<DeviceSummary[]>([]); const loading = ref(true); const error = ref(''); const creating = ref(false)
...
onMounted(load)
async function created(device: DeviceSummary) { workspace.selectedDeviceId = device.deviceId; creating.value = false; await load() }
</script>
