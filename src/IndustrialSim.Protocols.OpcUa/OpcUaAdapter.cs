...
public sealed class OpcUaAdapter : IProtocolAdapter
{
    ...
    public async Task StartAsync(IDeviceRuntime runtime, ProtocolOptions options, CancellationToken cancellationToken = default) { cancellationToken.ThrowIfCancellationRequested(); _runtime = runtime ?? throw new ArgumentNullException(nameof(runtime)); Endpoint = options.Endpoint ?? "opc.tcp://0.0.0.0:4840"; IsRunning = true; _runtime.State.DataPointChanged += OnDataPointChanged; if (options.Port > 0) await StartServerAsync(options.Port, cancellationToken); }
